...
set _DeploySetParametersFile=%RootPath%CustomsDeclarationProxy.SetParameters.xml
)
...
@rem ---------------------------------------------------------------------------------
if not exist "%RootPath%CustomsDeclarationProxy.zip" (
echo "%RootPath%CustomsDeclarationProxy.zip" does not exist. 
...
echo. "%MSDeployPath%\msdeploy.exe" -source:package='%RootPath%CustomsDeclarationProxy.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -skip:objectname='dirPath',absolutepath='obj\\Debug\\Package\\PackageTmp\\App_Data$' %_MsDeployAdditionalFlags%
"%MSDeployPath%\msdeploy.exe" -source:package='%RootPath%CustomsDeclarationProxy.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -skip:objectname='dirPath',absolutepath='obj\\Debug\\Package\\PackageTmp\\App_Data$' %_MsDeployAdditionalFlags%
) else (
echo. "%MSDeployPath%\msdeploy.exe" -source:package='%RootPath%CustomsDeclarationProxy.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -skip:objectname='dirPath',absolutepath='obj\\Debug\\Package\\PackageTmp\\App_Data$' -setParamFile:"%RootPath%CustomsDeclarationProxy.SetParameters.xml" %_MsDeployAdditionalFlags%
...
:usage
echo =========================================================
if not exist "%RootPath%CustomsDeclarationProxy.deploy-readme.txt" (
...
start notepad "%RootPath%CustomsDeclarationProxy.deploy-readme.txt"
)
echo =========================================================
goto :eof
...
:CheckParameterFile
echo =========================================================
if  exist "%RootPath%CustomsDeclarationProxy.SetParameters.xml" (
echo SetParameters from:
echo "%RootPath%CustomsDeclarationProxy.SetParameters.xml"
echo You can change IIS Application Name, Physical path, connectionString
echo or other deploy parameters in the above file.
) else (
echo SetParamterFiles does not exist in package location.
